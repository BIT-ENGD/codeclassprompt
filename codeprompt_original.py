...
def build_optimizer_ex(args,model,batch_size,epoch,train_dataloader):
    ...
    warm_up_ratio = args.warmup_ratio*total_steps # 定义要预热的step ，  warm_up_ratio * total_steps
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps = warm_up_ratio, num_training_steps = total_steps)
...
def codepromt_one_dataset(dsinfo,args,label_words_path,template,tempid):
    ...
    if args.model_name == "codeprompt":
        ...
        prompt_model = CodePromptAttentionTest(plm=plm,template=mytemplate, verbalizer=myverbalizer,freeze_plm=False, plm_eval_mode=args.plm_eval_mode,args=args).to(args.device)
    ...
    train_dataloader = PromptDataLoader(dataset=dsinfo["support"], template=mytemplate, tokenizer=tokenizer,
                                 tokenizer_wrapper_class=WrapperClass, max_seq_length=dsinfo["max_seq"], decoder_max_length=3,
                                  batch_size=args.shot*10,shuffle=False, teacher_forcing=False, predict_eos_token=False,
    ...
    for epoch in range(args.max_epochs):
        ...
        with torch.autograd.set_detect_anomaly(True):
            for step, inputs in enumerate(pbr):
                inputs = inputs.to(args.device)
                test=np.mean(inputs["label"].tolist())
                # if np.mean(inputs["label"].tolist()) != inputs["label"][0].item():
                #     raise ValueError("not same label!!")
                logits = prompt_model(inputs).squeeze(0)
...
def main(): 
   ...
   # sort_sample_by_class(dsinfo['support'])
